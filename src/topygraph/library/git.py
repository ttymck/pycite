...
class GitLibrary:
    ...
    def _load_git_projects_from_catalog(
    ...
    def _set_git_pull_cache(self, git_projects) -> None:
        ...
        output = {
            ...
            "project_names": [p.name for p in git_projects],
        }
        ...
        if self.git_pull_cache_file.exists():
            try:
                with open(self.git_pull_cache_file, "r") as f:
                    cache = json.load(f)
                cached_timestamp = datetime.strptime(
                    cache.get("timestamp"), "%Y-%m-%dT%H:%M:%S"
                )
                if cached_timestamp + self.cache_ttl >= datetime.now():
                    logger.debug(
                        "Using cached git projects from %s", cache.get("timestamp")
                    )
    ...
    def pyglob(self) -> typing.List[GitProject]:
        ...
        self._pyglob()
        return [
            GitProjectGlob(project.name, project.pyglob) for project in self.projects
        ]
